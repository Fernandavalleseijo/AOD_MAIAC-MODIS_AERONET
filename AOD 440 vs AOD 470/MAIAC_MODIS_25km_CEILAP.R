...
Celiap_window_25km <- read_sf(paste(ROI_path,'Ceilap_win_25km.kml', sep = ""))
...
Celiap_window_sinu <- st_transform(Celiap_window_25km, crs=proj_sinu)

# To use in terra:crop (Because rasters are in sinusoidal projection)
extent = terra::ext(Celiap_window_sinu)
...
write.table(filtered_df, paste0(csv_path, "MAIAC_MODIS_25KM_CELIAP.csv"), sep = ",", row.names = FALSE)
